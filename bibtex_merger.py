...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Merge duplicate BibTeX entries and re-key them."
    )
    ...
    parser.add_argument(
        "--conflicts-bib",
        help="Optional path to write conflicts + chosen solutions as .bib",
    )
    parser.add_argument(
        "--apply-corrections",
    ...
    args.input_bib = resolve_existing_bib_path(args.input_bib)
    if not args.output_bib:
        args.output_bib = default_output_path(args.input_bib)
    if not args.report:
        args.report = derive_default_path(args.input_bib, "_conflicts.txt")
    if not args.conflicts_bib:
        args.conflicts_bib = derive_default_path(args.input_bib, "_conflicts.bib")

    with open(args.input_bib, "r", encoding="utf-8") as handle:
        text = handle.read()

    entries, passthrough = parse_bibtex(text)
    correction_lines: List[str] = []
    correction_path = resolve_corrections_path(args.apply_corrections, args.input_bib)
    ...
    if correction_path:
        with open(correction_path, "r", encoding="utf-8") as handle:
            corrections_text = handle.read()
        corrections, _ = parse_bibtex(corrections_text)
        correction_lines = apply_corrections(entries, corrections)
        if not args.corrected_output:
            args.corrected_output = derive_default_path(args.input_bib, "_corrected.bib")
        write_bibtex(args.corrected_output, entries, passthrough)

    merged_entries, report_lines, conflict_clusters = merge_entries(entries)
    write_bibtex(args.output_bib, merged_entries, passthrough)

    if args.conflicts_bib:
        write_conflicts_bib(args.conflicts_bib, conflict_clusters, entries)
